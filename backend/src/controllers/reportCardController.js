...
// ==================== GET GRADE SUMMARY (FIXED - WITH SECTION) ====================
...
const getStudentReportData = async (req, res) => {
    try {
        ...
        const average = subjectsWithGrades > 0 ? totalPercentage / subjectsWithGrades : 0;
    ...
    try {
        ...
        let query = `
            ...
            SET 
                ...
                published_at = CURRENT_TIMESTAMP
            WHERE student_id IN (
                SELECT id FROM students WHERE grade_level = $1 AND status = 'ACTIVE'
            )
            AND semester = $2
            AND academic_year = $3
        `;
